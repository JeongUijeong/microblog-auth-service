...
public class AuthService {
    ...
    /**
     * Refresh token으로 access token 재발급(토큰 회전)
     */
    public TokenResponse refreshAccessToken(String refreshToken) {
        // 1) signature/expiry 검사
        if (!jwtProvider.validateToken(refreshToken)) {
            throw new IllegalArgumentException("Invalid refresh token");
        }

        // 2) jti 추출 -> DB에 존재하는지 확인
        ...
        // 4) 새 access, 새 refresh 생성 (rotate)
        ...
        refreshTokenRepository.save(new RefreshToken(newJti, stored.getUserId(), newRefreshClaims.getExpiration().toInstant()));
